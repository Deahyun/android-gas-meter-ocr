...
- **API 키가 앱 안에 들어갑니다.** `BuildConfig` 로 옮겨 소스에서는 분리했지만, 빌드된 APK 를 디컴파일하면 키를 꺼낼 수 있습니다. 실제 서비스라면 앱이 직접 OpenAI 를 호출하지 않고, 키를 보관하는 서버를 두어 앱은 그 서버만 호출해야 합니다. 이 저장소는 인식 방식 확인이 목적이라 단순한 구성을 택했습니다.
